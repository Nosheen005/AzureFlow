...
# Sidebar (only one mart now)
page = st.sidebar.radio("Meny", ["Start", "Annonser"])

# Always load the construction mart
df_test = query_job_listings("SELECT * FROM marts.mart_construction")
...
def layout_graphs(df: pd.DataFrame, mart: str, name: str):
    ...
    # Aggregates (connector returns UPPERCASE columns)
    ...
    # Order for charts
    def order_frame(df_, cat_col):
        df_ = df_.sort_values("VACANCIES", ascending=False)
        df_[cat_col] = pd.Categorical(df_[cat_col], categories=df_[cat_col], ordered=True)
        return df_

    df_employer  = order_frame(df_employer,  "EMPLOYER_NAME")
    df_occupation = order_frame(df_occupation, "OCCUPATION")
    df_region    = order_frame(df_region,    "WORKPLACE_REGION")
    df_duration  = order_frame(df_duration,  "DURATION")
...
def layout_ads(df: pd.DataFrame):
    ...
    with cols[1]:
        select_occupation = st.selectbox(
            "Select occupation:",
            df.query("WORKPLACE_REGION == @select_region")["OCCUPATION"].dropna().unique()
        )
    with cols[2]:
        select_company = st.selectbox(
            "Select company:",
            df.query("WORKPLACE_REGION == @select_region & OCCUPATION == @select_occupation")["EMPLOYER_NAME"].dropna().unique()
        )
    with cols[3]:
