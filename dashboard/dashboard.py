...
def layout_graphs(df, mart, name):
    ...
    # ensure VACANCIES usable
    if "VACANCIES" not in df.columns:
        st.warning("Kolumnen 'VACANCIES' saknas i data.")
        return
    ...
    # ---- Aggregations (correct identical indentation) ----
    df_employer = query_job_listings(
        f"""
            SELECT SUM(vacancies) AS vacancies, employer_name
            FROM {mart}
    ...
    if df_employer.empty or df_occupation.empty or df_region.empty:
        st.warning("Ingen data hittades för denna kategori.")
        return
...
def layout_ads(df):
    if df.empty:
        st.warning("Ingen data tillgänglig att visa.")
        return

    cols = st.columns(4)
    with cols[0]:
        select_region = st.selectbox('Select region:', df["WORKPLACE_REGION"].dropna().unique())
    with cols[1]:
        select_occupation = st.selectbox('Select occupation:', df.query('WORKPLACE_REGION == @select_region')["OCCUPATION"].dropna().unique())
    with cols[2]:
        select_company = st.selectbox('Select company:', df.query('WORKPLACE_REGION == @select_region & OCCUPATION == @select_occupation')["EMPLOYER_NAME"].dropna().unique())
...
if page2 == "Bygg och anläggning":
    ...
    # --- tiny diagnostic (shows once, does not change layout) ---
    if df_test.empty:
        diag = query_job_listings('SELECT COUNT(*) AS n FROM marts.mart_economics')
        st.warning(f"Administration-mart har {int(diag['N'].iloc[0]) if 'N' in diag.columns else 0} rader i {mart}. Kontrollera dbt build och DUCKDB_PATH.")
